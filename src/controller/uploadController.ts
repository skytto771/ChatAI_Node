...
export const uploadSmall = async (req: Request, res: Response) => {
  try {
    ...
    if (existFile) {
      return res.json(ResponseUtil.success(existFile.toJSON(), "文件已存在"));
    }
    ...
    // 更新文件记录：设置物理路径和访问 URL
    await file.update({ filePath: finalPath, fileUrl: accessUrl });

    res.json(ResponseUtil.success(file.toJSON(), "上传成功"));
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json(ResponseUtil.error(BusinessCode.SYSTEM_ERROR, "上传失败"));
  }
};
...
export const uploadChunk = async (req: Request, res: Response) => {
  try {
    ...
    const fileChunk = await FileService.saveChunk(
      fileId,
      chunkIndex,
      chunkPath,
      chunkSize,
...
export const uploadComplete = async (req: Request, res: Response) => {
  try {
    ...
    const isComplete = await FileService.checkChunksComplete(
      fileId,
      chunkCount,
    );
    if (!isComplete) {
    ...
    const { finalDir, backDir, yearMonth } = FileStorageService.getStoragePath(
      file.file.mimeType,
    );
    ...
    const isValid = await FileStorageService.verifyFileIntegrity(
      finalPath,
      file.file.fileHash,
    );
    if (!isValid) {
