...
// ========== 2. 创建时可选的字段 ==========
export interface ConversationCreationAttributes extends Optional<ConversationAttributes, 'id' | 'title' | 'systemPrompt' | 'tokenCount' | 'isArchived'> {}
...
export class Conversation extends Model<ConversationAttributes, ConversationCreationAttributes> implements ConversationAttributes {
     ...
     * 获取用户的活跃会话列表
     */
    static async getActiveConversations(userId: number, limit: number = 20): Promise<Conversation[]> {
     ...
     * 获取用户的归档会话列表
     */
    static async getArchivedConversations(userId: number): Promise<Conversation[]> {
        return await Conversation.findAll({
            where: {
                userId,
                isArchived: true,
     ...
     * 搜索用户的会话
     */
    static async searchConversations(userId: number, keyword: string): Promise<Conversation[]> {
...
export default function initConversation(sequelize: Sequelize): typeof Conversation {
    Conversation.init(
        {
            id: {
