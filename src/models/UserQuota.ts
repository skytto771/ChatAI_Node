...
// ========== 2. 创建时可选的字段 ==========
export interface UserQuotaCreationAttributes extends Optional<UserQuotaAttributes, 'id' | 'tokenLimit' | 'tokensUsed'> {}

// ========== 3. 扩展 Model 类 ==========
export class UserQuota extends Model<UserQuotaAttributes, UserQuotaCreationAttributes> implements UserQuotaAttributes {
    declare id: number;
     ...
     * 查找或创建用户额度记录
     */
    static async findOrCreateQuota(userId: number, tokenLimit: number = 1000000): Promise<UserQuota> {
     ...
     * 尝试消耗 Token，返回是否成功
     */
    static async tryConsumeTokens(userId: number, tokens: number): Promise<{ success: boolean; quota: UserQuota }> {
...
export default function initUserQuota(sequelize: Sequelize): typeof UserQuota {
    UserQuota.init(
        ...
        },
        {
            ...
            paranoid: false,
            indexes: [
                { unique: true, fields: ['user_id'], name: 'idx_user_id' },
            ],
            hooks: {
