...
// ========== 2. 创建时可选的字段 ==========
export interface MessageFileCreationAttributes extends Optional<MessageFileAttributes, 'id'> {}

// ========== 3. 扩展 Model 类 ==========
export class MessageFile extends Model<MessageFileAttributes, MessageFileCreationAttributes> implements MessageFileAttributes {
    declare id: number;
     ...
     * 批量创建消息文件关联
     */
    static async batchCreate(messageId: number, fileIds: number[]): Promise<MessageFile[]> {
     ...
     * 获取某条消息的所有文件关联
     */
    static async getFilesByMessageId(messageId: number): Promise<MessageFile[]> {
...
export default function initMessageFile(sequelize: Sequelize): typeof MessageFile {
    MessageFile.init(
        {
            id: {
